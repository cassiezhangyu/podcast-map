...
knowledge.md 是用户理解材料的主入口，不藏在画廊之后。完整知识包的网页或最终回复先呈现其“快速理解”内容、完整母稿入口和证据边界，再列视觉成品；封面是传播入口，不是知识入口。母稿和公开页面的陌生读者表达按 [reader-first-writing.md](reader-first-writing.md) 检查。
...
- 保持来源特有语言与句子节奏；按 [reader-first-writing.md](reader-first-writing.md) 检查语境依赖、重复句式和口号式收束，不用禁用词表冒充自然表达；
- 把字符数和段落—图片映射放在内部审计区，不混入公开正文。

平台限制无法实时核验时，采用保守长度并在内部记录“未核验”，不得宣称为官方上限。
- 公开正文末尾可附少量与内容直接相关的话题标签；标签不计入连续叙事，也不替代内部覆盖审计。
